void doViz( int[][][] vars) {
  ...
  fill( 0, 62);
    ...
    int posX = int( map( highestPoint.x, 0, flexValues.length, 0, width));
    float posY = height - highestPoint.y * height;    
    
    drawBezierVertexBetween( 0, height, posX, posY);
    drawBezierVertexBetween( posX, posY, width, height);
